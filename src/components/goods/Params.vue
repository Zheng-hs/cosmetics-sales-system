      ...
      >
        ...
        <el-table-column type="expand">
          <template slot-scope="scope">
            <!-- 循环渲染tag标签 -->
            <el-tag
              v-for="(item, i) in scope.row.goodsNormsEntities"
              :key="i"
              closable
  ...
  methods: {
    async getCateList() {
      const { data: res } = await this.$http.post(
        `/api/v1/goodsNorms/search?pageNo=${this.queryInfo.pageNo}&pageSize=${this.queryInfo.pageSize}`,
        {}
      );
      if (res.code !== 200) {
