    ...
    <el-card>
      ...
      >
        <el-row :gutter="22">
          ...
          <el-col :span="12">
            <el-form-item label="商品轮播图" class="picture">
              <div
                class="imglist"
                v-for="(item, i) in subImageList"
                :key="i"
              ...
              >
                ...
                <span
                v-show="isDelete"
                  :class="{active: activeName == item,delete1:activeName !== item}"
          ...
          <el-table-column label="原价" prop="goodsOriginPrice">
            <template slot-scope="scope">
              <el-input
                ...
                @keyup.enter.native="
                  handleCurrentChange(scope.row, scope.$index)
                "
...
export default {
  ...
  methods: {
    async getGoodsInfo() {
      ...
      this.imageUrl = this.addForm.mainImage;
      this.subImageList = this.addForm.subImageList;
      console.log(this.addForm);
      const { data: res } = await this.$http.post(
        "/api/v1/goodsClassify/search",
        {
          classifyId: this.addForm.classifyParentId,
        }
      );
